...
- SecOps/ChatOps アーキテクチャ（イベント駆動 / セキュリティ運用 / インフラ自動化）
- 構成: Xserver → Fail2Ban / Logwatch → AWS API Gateway → Lambda → SNS → Microsoft Teams Workflow
- Fail2BanとLogwatchを用いた不正アクセス検知・IP遮断の自動化
- AWS Lambdaによるイベント駆動アラート処理とMicrosoft Teams Workflowへのリアルタイム通知
